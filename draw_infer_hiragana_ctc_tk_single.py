...
def preprocess_for_infer(img_L: Image.Image, cfg: InferConfig, bbox) -> Tuple[torch.Tensor, int, Image.Image]:
    """
    学習 (train_hiragana_crnn_ctc.py) と整合する前処理：

    - strokes_bbox で粗く切る (pad=0)
    - 画素ベースで tight ink bbox を取り直して「余白ゼロ」 crop
    - blur（必要なら）
    - 高さ target_h にリサイズ（幅は可変）
    - max_w=512 へ右パッド（学習の collate と同じ入力幅に揃える）
    - valid_w_px は「インク右端」で推定（学習の estimate_valid_width_from_image_L 相当）

    returns:
      x: [1,1,H,W] float32 in [0,1] (W==max_w)
      valid_w_px: 有効幅(px) = インクの右端+1 (<=max_w)
      out_L: デバッグ用の最終入力画像（L, W==max_w）
    """
    if bbox is None:
        raise ValueError("Nothing drawn.")
    ...
    # 5) max_w 超過なら右を切る（学習と同じ）
    if resized.size[0] > cfg.max_w:
        resized = resized.crop((0, 0, cfg.max_w, resized.size[1]))

    # 6) ★学習の collate でやっているのと同様に「右パッドで max_w に揃える」
    padded = _pad_to_max_w(resized, cfg.max_w, bg=255)

    # 7) ★valid width を「インク右端」で推定（学習の valid_w と一致）
    arr01 = np.array(padded, dtype=np.float32) / 255.0  # [H,W] in [0,1]
    valid_w_px = estimate_valid_width_from_array(
        arr01, ink_thresh01=float(cfg.ink_thresh) / 255.0)
    valid_w_px = max(1, min(int(valid_w_px), int(cfg.max_w)))
...
class DrawInferApp:
    def __init__(self, model_ts: Path, vocab_json: Path, cfg: InferConfig, device: str = "cpu"):
        ...
        tk.Label(self.root, textvariable=self.result_var).grid(
            row=2, column=0, columnspan=6, padx=10, pady=10, sticky="w")

        # debug
        self._last_debug_input: Optional[Image.Image] = None
        self._last_valid_w_px: Optional[int] = None
    ...
    def clear(self):
        ...
        self._last_valid_w_px = None
        self.result_var.set("Cleared.")

    def infer(self):
        try:
            x, valid_w_px, final_L = preprocess_for_infer(
                self.img, self.cfg, self.strokes_bbox)
            self._last_debug_input = final_L
            self._last_valid_w_px = valid_w_px
            ...
            # cut to valid time steps (match training width_to_time_steps)
            ...
            self.result_var.set(
                f"Text: {text}   (valid_w_px={valid_w_px}, T_valid={T_valid})")
    ...
    def save_debug(self):
        try:
            ...
            out = Path("debug_hira_ctc_input_padded512.png")
            self._last_debug_input.save(out)

            vw = self._last_valid_w_px
            self.result_var.set(f"Saved {out}   (valid_w_px={vw})")
        except Exception as e:
            self.result_var.set(f"Error: {e}")
...
def main():
    ...
    # NOTE: used only for legacy; preprocess uses pad=0
